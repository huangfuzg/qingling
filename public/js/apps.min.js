...
var handleHeaderFixedTop = function() {
   ...
   pc_TurnImgs = function(cata) {
	...
	pc_insertTables = function(cata){	
		$.getJSON("parameter/pc.json", "", function(data) {
				...
				$.each(model,function(j,item){
				 ...
				 //        </div>';
					...
					$.each(smallimgs,function(i,item){
						...
						if(smallimgs[i]!=""){
							var pic = '<img src="parameter/AC POWER CORD/'+item+'" alt="">';
							$('.'+u+'_'+j+' h4').after(pic);
						}
					})
				...
				if(data[i].id === cata){
					...
					$.each(detail,function(i,item){
						...
						if(i%2===0){
							...
							<div class="image"><img src="parameter/AC POWER CORD/'+item.drawing+'" alt="" />\
							...
							if(item.picture!=""){
								...
								var str = '<div class="product-desc">\
		                          <div class="image1">\
		                              <img src="parameter/AC POWER CORD/'+item.picture+'" alt="" style="width: 100%" />\
		                          </div></div>';
		                        a.append(str);                    	
							}
						}
	...
	App = function() {
		...
		return {
			...
			init_pc: function(){
				...
				mobileNav(),pc_title(type),pc_TurnImgs(type),pc_insertContent(type),show_sidebar(type,pname,cata),pc_insertTables(type)
				//pc_insertLineImgs(type)
				//$('.pgwSlideshow').pgwSlideshow(option);
			},
			init_other: function(){
