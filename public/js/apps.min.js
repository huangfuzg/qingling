...
var handleHeaderFixedTop = function() {
   ...
   pc_TurnImgs = function(cata) {
	...
	pc_insertTables = function(){	
		$.getJSON("parameter/powercord.json", "", function(data) {
			//console.log(data[0].cable);
			$.each(data[0].cable,function(i,item){
				console.log(i,item);
				var name = item.name;
				var a = $('#product-desc');
				...
				$.each(model,function(j,item){
					...
					var uname = item.name;
					console.log(j,item);
					var head = '<div class="linepic '+u+'-'+j+'">\
                                  <h4 style="text-align: center">'+item.name+'</h4>\
                                  <img src="data/imgs/ac_power/china_ac_power/picture/model1.jpg" alt="">\
                                </div>';
					b.after(head);
	...
	App = function() {
		...
		return {
			init_mpowercord:function() {
				...
				mobileNav(),pc_title(type),pc_insertNav(type),pc_mTurnImgs(type),pc_insertContent(type),pc_insertTables()
			...
			init_pc: function(){
				...
				mobileNav(),pc_title(type),pc_TurnImgs(type),pc_insertContent(type),show_sidebar(type,pname,cata),pc_insertTables()
				//pc_insertLineImgs(type)
				//$('.pgwSlideshow').pgwSlideshow(option);
			},
			init_other: function(){
